...
router.post('/login', (req, res) => {
  ...
  db.query('SELECT * FROM donors WHERE email = ?', [email], (err, results) => {
    ...
    const token = jwt.sign({ id: donor.id, email: donor.email }, process.env.JWT_SECRET || 'bloodbank_secret', { expiresIn: '24h' })
    res.json({
      message: 'Login successful',
      token,
      donor: { id: donor.id, full_name: donor.full_name, email: donor.email, blood_type: donor.blood_type, is_eligible: donor.is_eligible }
    })
  })
})
...
router.put('/change-password', (req, res) => {
  ...
  db.query('SELECT * FROM donors WHERE id = ?', [donor_id], (err, results) => {
    if (err) return res.status(500).json({ message: err.message })
    if (results.length === 0) return res.status(404).json({ message: 'Donor not found' })
    const donor = results[0]
    const match = bcrypt.compareSync(old_password, donor.password)
    ...
    (err) => {
      ...
      db.query(
        `UPDATE blood_requests SET quantity_needed = quantity_needed - 1
         WHERE hospital_id = ? AND blood_type = ? AND quantity_needed > 0 AND status = 'pending' LIMIT 1`,
