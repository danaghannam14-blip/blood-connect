...
function Dashboard() {
 ...
 const markDonated = async (id, hospital_id, blood_type) => {
  ...
  const getDonationWarning = (currentTotal) => {
  if (currentTotal === 0) return { msg: '💡 Max allowed: 2 units total. Your first donation saves up to 3 lives!', color: 'text-gray-500' }
  if (currentTotal === 1) return { msg: '⚠️ You donated 1 unit already. You can donate 1 more unit. Remember to stay hydrated!', color: 'text-yellow-600' }
  return { msg: '🛑 You reached the maximum of 2 units. Please rest before donating again!', color: 'text-red-600' }
}

  if (!donor) return null

  const warning = getDonationWarning()
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow p-6 mb-6">
          ...
          {notifications.length === 0 && history.length === 0 ? (
              ...
              {notifications.filter(n => !n.donated).map(n => (
                <div key={n.id} className="border-b py-3 last:border-0">
                  ...
                  <div className="mt-2 bg-red-50 rounded-xl p-3">
                    <p className={`text-xs mb-2 ${warning.color}`}>{warning.msg}</p>
           ...
           {notifications.filter(n => !n.donated).map(n => {
  const donatedSoFar = notifications.filter(x => x.donated).length + history.length
