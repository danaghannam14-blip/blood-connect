  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <nav className="flex justify-between items-center px-5 py-4 shadow-sm bg-white sticky top-0 z-50">
        ...
        <div className="hidden md:flex gap-3 items-center">
          <button onClick={() => navigate('/inventory')}
            className="text-gray-600 hover:text-red-600 text-sm font-medium">Blood Status</button>
      ...
      {menuOpen && (
        <div className="md:hidden bg-white border-b shadow-sm px-5 py-4 flex flex-col gap-3 z-40">
          <button onClick={() => { navigate('/inventory'); setMenuOpen(false) }}
            className="text-gray-700 text-sm font-medium text-left py-2 border-b border-gray-100">Blood Status</button>
          <button onClick={() => { navigate('/how-it-works'); setMenuOpen(false) }}
            className="text-gray-700 text-sm font-medium text-left py-2 border-b border-gray-100">How It Works</button>
          <button onClick={() => { navigate('/emergency'); setMenuOpen(false) }}
            className="text-red-600 text-sm font-semibold text-left py-2 border-b border-gray-100">Emergency</button>
          <button onClick={() => { navigate('/login'); setMenuOpen(false) }}
            className="bg-red-600 text-white px-4 py-2.5 rounded-xl text-sm font-semibold w-full">Sign In</button>
        </div>
      )}
      ...
      <footer className="bg-gray-900 py-6 px-6 text-center text-gray-500 text-sm">
        © 2026 BloodConnect · Dana Ghannam & Lynn Anani · Smart Donor Matching System
      </footer>
