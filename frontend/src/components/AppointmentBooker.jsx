...
function AppointmentBooker({ donor, onAppointmentsChange }) {
  ...
  useEffect(() => {
    axios.get(`${API}/api/requests/compatible/${donor.blood_type}`)
      .then(res => {
        // FIXED: Only show hospitals with PENDING requests and quantity_needed > 0
        const uniqueHospitals = []
        const seen = new Set()
        res.data.forEach(req => {
          // Filter: only pending status, quantity_needed > 0
          if (req.status === 'pending' && req.quantity_needed > 0 && !seen.has(req.hospital_id)) {
