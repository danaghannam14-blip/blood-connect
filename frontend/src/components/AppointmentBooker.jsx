...
function AppointmentBooker({ donor, onAppointmentsChange }) {
  ...
  // ✅ FIXED: Load ALL requests, not just one per hospital
  useEffect(() => {
    axios.get(`${API}/api/requests/compatible/${donor.blood_type}`)
      .then(res => {
        // Show ALL requests, not just unique hospitals
        const hospitalRequests = res.data.map(req => ({
          id: req.hospital_id,
          name: req.hospital_name,
          address: req.hospital_address,
          blood_type: req.blood_type,
          quantity_needed: req.quantity_needed,
          request_id: req.id  // Keep track of request ID
        }))
        setHospitals(hospitalRequests)
        console.log('Loaded blood requests:', hospitalRequests.length)
      }).catch(err => {
...
<p className="text-gray-400 text-sm">No hospitals currently need your blood type.</p>
                ...
                // ✅ FIX 2: Use unique key and request_id for individual selection
<button key={`${h.id}-${h.blood_type}-${index}`} type="button"
                  onClick={() => setForm({ ...form, hospital_id: h.request_id, time: '' })}
                  className={`w-full text-left px-4 py-3 rounded-xl border transition-all
                    ${form.hospital_id === h.request_id
                      ? 'bg-red-600 border-red-600 text-white'
                      : 'bg-white border-gray-200 hover:border-red-300 text-gray-800'}`}>
<p className="font-semibold text-sm">{h.name}</p>
<p className={`text-xs mt-0.5 ${form.hospital_id === h.request_id ? 'text-red-200' : 'text-red-500'}`}>
                    🩸 Needs {h.quantity_needed} units of {h.blood_type}
</p>
                  {h.address && (
<p className={`text-xs mt-0.5 ${form.hospital_id === h.request_id ? 'text-red-100' : 'text-gray-400'}`}>
                      📍 {h.address}
</p>
                  )}
</button>
